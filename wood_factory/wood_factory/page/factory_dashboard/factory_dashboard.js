...
function render_dashboard(page, data) {
    ...
    const stages = page.main.find(".stage-board"); Object.entries(data.stage_counts || {}).forEach(([stage, count]) => stages.append(`<button class="stage-tile" data-stage="${frappe.utils.escape_html(stage)}"><span>${__(stage)}</span><b>${count}</b></button>`));
    page.main.append(section(__("Delayed Orders"), delayed_rows(data.delayed_orders || []))); page.main.append(section(__("Blocked Work"), blocked_rows(data.blocked_stages || []))); page.main.append(section(__("Open Piece Exceptions"), exception_rows(data.exceptions || [])));
    ...
    page.main.find("[data-order]").on("click", function () { frappe.set_route("Form", "Factory Order", $(this).data("order")); }); page.main.find("[data-exception]").on("click", function () { frappe.set_route("Form", "Piece Exception", $(this).data("exception")); }); page.main.find("[data-reference-type]").on("click", function () { frappe.set_route("Form", $(this).data("reference-type"), $(this).data("reference-name")); });
}
