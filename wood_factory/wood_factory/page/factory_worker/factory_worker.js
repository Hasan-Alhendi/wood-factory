...
function piece_card(piece) { return `<div class="worker-card worker-exception" data-kind="piece" data-name="${frappe.utils.escape_html(piece.name)}"><div class="worker-card-body"><h4>${frappe.utils.escape_html(piece.part_name || piece.piece_uid)}</h4><div>${piece.factory_order} · ${piece.width_mm} × ${piece.height_mm} mm · <b>${__(piece.current_stage)}</b></div><small>${frappe.utils.escape_html(piece.workstation || __("Assign on start"))} · ${__("Separate piece")}: ${__(piece.status)} · ${__("Cumulative costing")}: ${__(piece.costing_status || "Pending")}${piece.block_reason ? ` · ${frappe.utils.escape_html(piece.block_reason)}` : ""}${piece.last_stage_actual_minutes ? ` · ${__("Last stage")}: ${piece.last_stage_actual_minutes} min / ${__(piece.last_stage_costing_status || "Pending")}` : ""}</small></div>${action_buttons(piece.status)}</div>`; }
