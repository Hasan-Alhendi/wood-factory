...
function render_schedule(page, data) {
    ...
    page.main.html(`<div class="schedule-kpis">${kpi(__("Queued Orders"),s.queued)}${kpi(__("Urgent"),s.urgent,"urgent")}${kpi(__("High Priority"),s.high,"high")}${kpi(__("With Exceptions"),s.with_exceptions,"warning")}</div><div class="schedule-list">${rows.map(order=>`<button class="schedule-row priority-${(order.priority||"normal").toLowerCase()}" data-order="${order.name}"><strong class="queue-number">#${order.queue_position}</strong><div><b>${order.name}</b><span>${frappe.utils.escape_html(order.customer||"")}</span></div><div><b>${__(order.priority)}</b><span>${order.priority_override?__("Manual override"):__("Automatic priority")}</span></div><div><b>${__(order.current_stage||order.status)}</b><span>${order.progress_percent||0}%</span></div><div><b>${delivery_text(order)}</b><span>${order.open_exceptions} ${__("open exception(s)")}</span></div></button>`).join("")||`<div class="schedule-empty">${__("No orders waiting for production")}</div>`}</div>`);
