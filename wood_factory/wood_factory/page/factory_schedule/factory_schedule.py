...
def get_production_schedule():
    ...
    return {"orders": orders, "summary": {"queued": len(orders), "urgent": sum(1 for row in orders if row.priority == "Urgent"), "high": sum(1 for row in orders if row.priority == "High"), "with_exceptions": sum(1 for row in orders if row.open_exceptions), "unassigned": sum(1 for row in orders if row.current_stage and not row.workstation), "at_risk": sum(1 for row in orders if row.eta_risk in ("At Risk", "Late"))}}


def _apply_eta_forecast(orders):
    ...
    stage_rows = frappe.db.sql("""
        select parent, idx, stage, workstation, status, started_at, actual_minutes from `tabFactory Order Stage`
        where parent in %(orders)s and status not in ('Completed','Skipped') order by parent, idx
    """, {"orders": [row.name for row in orders] or [""]}, as_dict=True)
    stages_by_order = {}
    ...
    ordered = sorted(orders, key=lambda row: row.queue_position)
