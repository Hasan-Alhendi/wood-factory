...
def get_production_schedule():
    ...
    bottlenecks = _get_bottleneck_analysis()
    recommendations = _get_production_recommendations(orders=orders, bottlenecks=bottlenecks)
...
def get_bottleneck_analysis():
    require_planning()
    return _get_bottleneck_analysis()


def _get_bottleneck_analysis():
    ...
    workstations = frappe.get_all(
        ...
        fields=["name", "stage", "effective_minutes_per_day"],
        limit_page_length=0,
    )
    stage_capacity = {}
    ...
    for row in rows:
        ...
        data = stages.setdefault(row.stage, {
            "stage": row.stage, "load_minutes": 0, "waiting_orders": 0,
            "blocked_orders": 0, "in_progress": 0, "urgent_orders": 0, "workstations": {},
        })
    ...
    for stage, data in stages.items():
        ...
        for name, load in data["workstations"].items():
            ...
            station_rows.append({
                ...
                "capacity_minutes": station_capacity,
                "load_percent": round(load / station_capacity * 100, 1) if station_capacity else 999,
            })
...
def get_production_recommendations(orders=None, bottlenecks=None):
    require_planning()
    return _get_production_recommendations(orders=orders, bottlenecks=bottlenecks)


def _get_production_recommendations(orders=None, bottlenecks=None):
    orders = orders or _get_scheduled_orders()
    if orders and not orders[0].get("estimated_completion"):
        _apply_eta_forecast(orders)
    bottlenecks = bottlenecks or _get_bottleneck_analysis()
    recommendations = []
...
def simulate_production(workstation=None, downtime_days=0, urgent_order=None, urgent_minutes=0):
    ...
    return {"scenario": scenario, "impacts": impacts, "summary": {
        "affected": len(impacts),
        "new_at_risk": sum(1 for row in impacts if row["before_risk"] not in ("At Risk", "Late") and row["after_risk"] in ("At Risk", "Late")),
        "late_after": sum(1 for row in orders if row.eta_risk == "Late"),
        "max_shift_hours": max([row["shift_hours"] for row in impacts] or [0]),
    }}
...
def _get_scheduled_orders():
    orders = frappe.get_all(
        ...
        fields=["name", "customer", "status", "priority", "priority_override", "priority_reason", "current_stage", "progress_percent", "expected_delivery_date", "delay_days", "modified"],
        limit_page_length=0,
    )
...
def _summary(orders):
    return {
        "queued": len(orders),
        "urgent": sum(1 for row in orders if row.priority == "Urgent"),
        "high": sum(1 for row in orders if row.priority == "High"),
        "with_exceptions": sum(1 for row in orders if row.open_exceptions),
        "unassigned": sum(1 for row in orders if row.current_stage and not row.workstation),
        "at_risk": sum(1 for row in orders if row.eta_risk in ("At Risk", "Late")),
    }
...
def _apply_eta_forecast(orders, scenario=None):
    ...
    history = dict(frappe.db.sql("""select stage, avg(actual_minutes) from `tabFactory Order Stage` where status='Completed' and actual_minutes > 0 group by stage"""))
    capacities = {row.name: flt(row.effective_minutes_per_day) for row in frappe.get_all("Factory Workstation", filters={"status": "Active"}, fields=["name", "effective_minutes_per_day"], limit_page_length=0)}
    ...
    for row in stage_rows:
        stages_by_order.setdefault(row.parent, []).append(row)
    ...
    for order in ordered:
        ...
        for stage in stages_by_order.get(order.name, []):
            ...
            if scenario.get("urgent_order") == order.name and not urgent_applied:
                minutes += max(flt(scenario.get("urgent_minutes")), 0)
                urgent_applied = True
            if stage.status == "In Progress" and stage.started_at:
                elapsed = max((get_datetime(now_datetime()) - get_datetime(stage.started_at)).total_seconds() / 60, 0)
                minutes = max(minutes - elapsed, 5)
            ...
            if stage.workstation:
                workstation_available[stage.workstation] = finish
            breakdown.append({"stage": stage.stage, "workstation": stage.workstation, "minutes": minutes, "start": start, "finish": finish})
            cursor = finish
        order["estimated_completion"] = cursor if breakdown else None
        order["eta_breakdown"] = breakdown
        delivery = get_datetime(order.expected_delivery_date) if order.expected_delivery_date else None
        if not breakdown:
            order["eta_risk"] = "No Forecast"
        elif delivery and cursor.date() > delivery.date():
            order["eta_risk"] = "Late"
        elif delivery and date_diff(delivery.date(), cursor.date()) <= 1:
            order["eta_risk"] = "At Risk"
        else:
            order["eta_risk"] = "On Track"
