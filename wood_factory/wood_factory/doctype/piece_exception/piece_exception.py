...
class PieceException(Document):
    ...
    def start_replacement(self):
        ...
        if self.replacement_piece:
            frappe.throw(f"Replacement piece {self.replacement_piece} already exists")
        ...
        replacement.last_stage_costing_status = "Pending"
    ...
    def create_replacement_cutting_order(self):
        ...
        if self.suggested_remnant:
            frappe.throw(f"Remnant {self.suggested_remnant} is already reserved for this replacement")
        if self.replacement_cutting_order:
            frappe.throw(f"Replacement Cutting Order {self.replacement_cutting_order} already exists")
        piece = frappe.get_doc("Factory Piece", self.replacement_piece)
        source = frappe.get_doc("Cutting Order", piece.cutting_order)
    ...
    def confirm_replacement_completed(self):
        ...
        if replacement.status != "Completed" or replacement.current_stage != "Completed":
    ...
    def resolve(self, resolution_notes):
        ...
        if self.replacement_piece and self.status != "Replacement Completed":
            frappe.throw("Complete the replacement piece before resolving this exception")
        original = frappe.get_doc("Factory Piece", self.factory_piece)
        if self.replacement_piece:
    ...
    def cancel_exception(self, resolution_notes):
        if self.status in ("Resolved", "Cancelled"):
            frappe.throw("Exception is already closed")
        if self.replacement_piece:
            frappe.throw("Cannot cancel an exception after replacement manufacturing has started")
        if not resolution_notes:
    ...
    def _summary(self):
        return {"name": self.name, "factory_piece": self.factory_piece, "factory_order": self.factory_order, "exception_type": self.exception_type, "status": self.status, "replacement_piece": self.replacement_piece, "suggested_remnant": self.suggested_remnant, "replacement_cutting_order": self.replacement_cutting_order}
