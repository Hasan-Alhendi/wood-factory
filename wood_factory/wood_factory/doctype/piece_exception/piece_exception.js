frappe.ui.form.on("Piece Exception", {
    refresh(frm) {
        ...
        if (!["Replacement Required", "Replacement In Production", "Replacement Completed"].includes(frm.doc.status)) frm.add_custom_button(__("Require Replacement"), () => run_exception_method(frm, "require_replacement"), __("Replacement"));
        if (frm.doc.status === "Replacement Required") frm.add_custom_button(__("Start Replacement Manufacturing"), () => run_exception_method(frm, "start_replacement"), __("Replacement"));
        if (frm.doc.status === "Replacement In Production" && frm.doc.replacement_piece) {
            ...
            if (!frm.doc.suggested_remnant && !frm.doc.replacement_cutting_order) frm.add_custom_button(__("Create Factory-Funded Cutting Order"), () => create_replacement_cutting(frm), __("Replacement"));
        ...
        if (frm.doc.suggested_remnant) frm.add_custom_button(__("Open Suggested Remnant"), () => frappe.set_route("Form", "Board Remnant", frm.doc.suggested_remnant), __("Replacement"));
        if (frm.doc.replacement_cutting_order) frm.add_custom_button(__("Open Replacement Cutting Order"), () => frappe.set_route("Form", "Cutting Order", frm.doc.replacement_cutting_order), __("Replacement"));
        if (frm.doc.status === "Replacement Completed") frm.add_custom_button(__("Open Replacement Piece"), () => frappe.set_route("Form", "Factory Piece", frm.doc.replacement_piece), __("Replacement"));
...
function close_exception(frm, method, label) { frappe.prompt([{fieldname: "resolution_notes", fieldtype: "Small Text", label, reqd: 1}], values => run_exception_method(frm, method, values), label, __("Confirm")); }
