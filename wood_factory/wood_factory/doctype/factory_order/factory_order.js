frappe.ui.form.on("Factory Order", {
    refresh(frm) {
        ...
        if (!active) return;
        if (active.status === "Ready" || active.status === "Blocked") {
...
function run_order_method(frm, method) {
    frm.call(method).then(() => frm.reload_doc());
}

function run_stage_method(frm, method, row) {
    frm.call(method, {row_name: row.name}).then(() => {
        frappe.show_alert({message: __("Production stage updated"), indicator: "green"});
        frm.reload_doc();
    });
}
