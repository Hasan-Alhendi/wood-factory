...
def validate_factory_acceptance(company=None):
	...
	_check(
		checks, "Factory roles are installed", all(frappe.db.exists("Role", role) for role in FACTORY_ROLES)
	)
...
def _validate_order_documents(checks, code, order):
	...
	_check(
		checks,
		f"{code}: Physical pieces exist",
		frappe.db.count("Factory Piece", {"factory_order": order}) > 0,
	)
...
def _validate_normal_cutting(checks, scenarios):
	...
	_check(
		checks,
		"NORMAL-CUTTING: order is waiting for cutting",
		bool(values and values.current_stage == "Cutting"),
	)
...
def _validate_delayed_blocked(checks, scenarios):
	...
	_check(
		checks,
		"DELAYED-BLOCKED: current stage is edge banding",
		bool(values and values.current_stage == "Edge Banding"),
	)
	_check(
		checks,
		...
		bool(
			frappe.db.exists(
				"Factory Order Stage", {"parent": order, "stage": "Edge Banding", "status": "Blocked"}
			)
		),
	)
	_check(
...
def _validate_remnant_replacement(checks, scenarios):
	...
	_check(
		checks,
		"DAMAGED-REMNANT: no new-board cutting order is needed",
		bool(values and not values.replacement_cutting_order),
	)
	...
	_check(
		checks,
		"DAMAGED-REMNANT: remnant material exactly matches the piece",
		bool(remnant and remnant.board_item == piece_item),
	)
	_check(
		checks,
		"DAMAGED-REMNANT: remnant points to the replacement piece",
		bool(remnant and remnant.reserved_for_piece == values.replacement_piece),
	)
	_check(
		checks, "DAMAGED-REMNANT: remnant state is reserved", bool(remnant and remnant.status == "Reserved")
	)
...
def _validate_new_board_replacement(checks, scenarios):
	...
	_check(
		checks,
		"WRONG-DIM-NEW-BOARD: order is an internal replacement",
		bool(cutting and cutting.order_type == "Internal Replacement"),
	)
	_check(
		checks,
		"WRONG-DIM-NEW-BOARD: customer is not billed",
		bool(cutting and not cint(cutting.customer_billable)),
	)
	_check(
		checks,
		"WRONG-DIM-NEW-BOARD: cutting order links to the exception",
		bool(cutting and cutting.piece_exception == exception),
	)
	_check(
		checks,
...
def _validate_costing_and_events(checks, scenarios):
	...
	_check(
		...
		frappe.db.count("Factory Cost Ledger", {"factory_order": ["in", order_names], "status": "Posted"})
		> 0,
	)
	_check(
		checks,
	...
	_check(
		checks, "Completed demo orders have usable costing data", partial < len(order_names), critical=False
	)
